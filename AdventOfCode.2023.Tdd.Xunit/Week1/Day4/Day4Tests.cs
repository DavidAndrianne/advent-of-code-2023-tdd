...
public class Day4Tests
{
    ...
    public void ScratchCardWinner_WithValidInput_CorrectCounts()
    {
        ...
        {
            {1, 1},  // 1 + no winning numbers
            {2, 2},  // 1 + won by card 1(+1)
            {3, 4},  // 1 + won by cards 1(+1) & 2(+2)
            {4, 8},  // 1 + won by cards 1(+1), 2(+2) & 3(+4)
            {5, 14}, // 1 + won by cards 1(+1), 3(+4) & 4(+8)
            {6, 1}   // 1 + no winning numbers
        };
